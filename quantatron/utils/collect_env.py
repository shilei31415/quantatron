...
def get_env_module():
    var_name = "QUANTATRON_ENV_MODULE"
...
def collect_env_info():
    ...
    try:
        import quantatron  # noqa

        data.append(
            ("quantatron", quantatron.__version__ + " @" + os.path.dirname(quantatron.__file__))
        )
    except ImportError:
        data.append(("quantatron", "failed to import"))
    except AttributeError:
        data.append(("quantatron", "imported a wrong installation"))

    try:
        import quantatron._C as _C
    except ImportError as e:
        data.append(("quantatron._C", f"not built correctly: {e}"))
        ...
        if has_cuda and sys.platform != "win32":
            try:
                so_file = importlib.util.find_spec("quantatron._C").origin
            except (ImportError, AttributeError):
                pass
            else:
                data.append(
                    ("quantatron arch flags", detect_compute_compatibility(CUDA_HOME, so_file))
                )
    else:
        ...
        if has_cuda and getattr(_C, "has_cuda", lambda: True)():
            data.append(
                ("quantatron arch flags", detect_compute_compatibility(CUDA_HOME, _C.__file__))
            )
...
if __name__ == "__main__":
    try:
        from quantatron.utils.collect_env import collect_env_info as f
